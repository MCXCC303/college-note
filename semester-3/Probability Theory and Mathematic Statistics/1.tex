...
\begin{eg}
    ...
    目标事件：$\text{C}_{M}^{m}\text{C}_{N-M}^{n-m}$ 

    概率： \[
        P=\frac{\text{C}_{M}^{m}\text{C}_{N-M}^{n-m}}{\text{C}_{N}^{n}},n=\max\left\{ 0,n-\left( N-M \right)  \right\}, \ldots,\min\left\{ n,M \right\} 
    .\] 

    2. 有放回：

    \[
    P=\frac{\text{C}_{n}^{m}M^m\left( N-M \right) ^{n-m}}{N^n}=\text{C}_{n}^{m}\left( \frac{M}{N} \right) ^m\left( 1-\frac{M}{N} \right) ^{n-m},m\in \left[ 0,n \right]
    .\] 
    注意到该概率为伯努利分布$\text{C}_{n}^{m}B\left( n,\frac{M}{N} \right) $
\end{eg}
匹配问题：
\begin{eg}
    麦克斯韦-玻尔兹曼统计问题：
    ...
    同理：\[
        P\left( B \right) =\frac{\text{C}_{N}^{n}}{N^n}
    .\] 
\end{eg}        
\subsubsection{几何概型}%
\label{subsub:几何概型}
