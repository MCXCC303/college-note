...
\begin{notation}
    ...
    2. 正则性：${\sum_{i=1}^{+\infty} p_{i}=1}$，即每一个点的概率都应该知道
\end{notation}
\begin{eg}
    保险问题
    ...
    \[
        P\left\{ X=40 \right\} =\mathrm{C}_{10000}^{40}0.005^{40}\cdot 0.995^{9960}\approx 2.143\times 10^{-2}
    .\] 
    ...
    \begin{notation}
        ...
        如果${\lim_{n \to +\infty} np_n=\lambda>0}$（极小但不为0），则：
        \[
            \lim_{n \to +\infty} \mathrm{C}_{n}^{k}p_n^{k}\left( 1-p_n \right) ^{n-k}=\mathrm{e}^{-\lambda}\frac{\lambda^{k}}{k!},k=0,1,2\ldots
        .\] 
    ...
    .\] 
    \[
        P\left\{ X=40 \right\} =\frac{50^{40}}{40!}\mathrm{e}^{-50}\approx 0.02
    .\] 

    2. 死亡人数不超过70的概率

    \[
        P\left\{ X\le 70 \right\} =\sum_{k=0}^{70} \mathrm{C}_{10000}^{k}0.005^{k}\cdot 0.995^{(10000-k)}
    .\] 
    \[
        =\sum_{k=0}^{70} \frac{50^{k}}{k!}\mathrm{e}^{-50} \left( \lambda=np=50 \right) 
    .\] 
\end{eg}
\begin{notation}
...
\begin{center}
    \begin{tikzpicture}
        % Axis
        \draw [->] (-1,0)--(-1,3) node [above] {$F\left( x \right) $};
        \draw [->] (-2,0)--(3,0) node [right] {$x$};
        \node [anchor=north east] at(-1,0) {$O$};
